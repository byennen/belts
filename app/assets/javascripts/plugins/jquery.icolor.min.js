...
(function ($) {
    ...
    p.M.prototype = {
        ...
        _initLayout: function () {
            ...
            if (this._opts.beforeInit) {
                go = this._opts.beforeInit();
            };
            if (!go) return;
            this._opts.col = this._opts.col || this.colors.length;
            var colNum = this.defaultColor ? 6 : (this._opts.col < 1 ? 1 : this._opts.col);
            ...
            if (this._opts.afterInit) this._opts.afterInit();
        },
        show: function () {
            var _this = this;
            var pos = this.$t.offset(),
                cbk = this._opts.onShow ? function () {
                    _this._opts.onShow(pos);
                } : null;
            this.$layout.css({
                left: pos.left,
                top: pos.top + this.$t.height()
            });
            if (this._opts.slide) {
        ...
        submit: function () {
            if (this._opts.onSelect) {
                this._opts.onSelect(this.curColor);
            };
    ...
    $.fn.icolor.isDark = function (c) {
        var colr = parseInt(c.substr(1), 12);
        return (colr >>> 12) + ((colr >>> 6) & 0x00ff) + (colr & 0x0000ff) < 500;
    };
})(jQuery);